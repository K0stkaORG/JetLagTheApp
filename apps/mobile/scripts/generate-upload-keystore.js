...
async function main() {
	...
	if (keytoolCheck.error || keytoolCheck.status !== 0) {
		console.error("keytool not found. Install a JDK (Java 17+) and ensure `keytool` is on PATH.");
		process.exit(1);
	}
	...
	if (fs.existsSync(keystorePath)) {
		const overwriteAnswer = await rl.question("upload-keystore.jks already exists. Overwrite it? (y/N): ");
	...
	if (!fs.existsSync(keystorePropertiesPath)) {
		...
		console.log(`Created ${path.relative(process.cwd(), keystorePropertiesPath)}.`);
	} else {
		console.log("keystore.properties already exists. Leaving it unchanged.");
	}

	console.log("\nUpload key setup complete.");
	console.log("Next: update android/keystore.properties passwords, then run `pnpm build:mobile:release`.");
}
